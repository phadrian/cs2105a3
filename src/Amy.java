...
class Amy {  // Alice is a TCP client
	...
	public Amy(String ipStr, String portStr) {
		...
		try {
			...
			System.out.println("Connected to Bryan on port " + bobPort);
		...
		// Receive the public key and digest and verify it
		boolean isVerified = receivePublicKey();
		
		if (isVerified) {
			// Set the public key of crypto back to Bob's public key
	...
	public boolean receivePublicKey() {
		...
		try {
			...
			this.pubKey = (PublicKey)fromBob.readObject();
			byte[] digest = (byte[])fromBob.readObject();
			result = this.crypto.verifyPublicKey(pubKey, digest);
		} catch (IOException e) {
			System.out.println("Error: cannot create input/output stream");
			System.exit(1);
		} catch (ClassNotFoundException e) {
			System.out.println("Error: cannot typecast to PublicKey/byte[]");
			System.exit(1);
		}
	...
	class Crypto {
		...
		public boolean verifyPublicKey(PublicKey pubKey, byte[] digest) {
			boolean result = false;
			try {
				// decrypt the md5 digest first
				Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
				cipher.init(Cipher.DECRYPT_MODE, this.pubKey);
				
				// get the public key and run the md5 algo on it
				MessageDigest md = MessageDigest.getInstance("MD5");
				md.update("bryan".getBytes());
				md.update(pubKey.getEncoded());
				result = Arrays.equals(cipher.doFinal(digest), md.digest());
			} catch (GeneralSecurityException gse) {
                System.out.println("Error: wrong cipher to decrypt public key");
                System.exit(1);
            }
